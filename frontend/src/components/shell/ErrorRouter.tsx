    ...
    onClick: async () => {
      if (path) {
        try {
          ...
          await invoke("open_in_file_manager", { path }).catch(() => {});
        } catch { /* browser mode */ }
      }
      dismiss();
    },
...
export function ErrorRouter() {
  const lastApiError = useUiStore((s) => s.lastApiError);
  const dismiss = () => useUiStore.getState().setLastApiError(null);

  if (!lastApiError || lastApiError.type !== "error") return null;

  const config = buildConfig(
    lastApiError.code,
    lastApiError.message,
    lastApiError.context,
    dismiss,
  );
