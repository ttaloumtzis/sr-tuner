...
import type { TrainingStartMessage, DatasetValidateResultMessage } from "../../lib/ipc-types";
...
const ARCH_OPTIONS: Architecture[] = ["Real-ESRGAN", "SwinIR", "HAT", "EDSR"];
...
export function ScreenTrainingSetup() {
  // §11.14 — hardware.info devices from uiStore
  const detectedDevices = useUiStore((s) => s.detectedDevices);
  ...
  // §11.9 — listen for dataset.validate.result
  useEffect(() => {
    let unlisten: (() => void) | null = null;
    listen<DatasetValidateResultMessage>("sidecar-message", ({ payload }) => {
      if (payload.type === "dataset.validate.result") {
        setDatasetValid(payload.valid);
        setDatasetErrors(payload.errors);
      }
    }).then((fn) => {
      unlisten = fn;
    });
  ...
  const sendValidate = useCallback(() => {
    ...
    sendToSidecar({
      ...
      training_path: hrPath,
      // §11.9: when strategy === "none", send validation_path: null
      validation_path: strategy === "none" ? null : (validationPath || null),
    });
  }, [hrPath, validationPath, strategy]);

  // §11.14 — GPU device dropdown options from hardware.info
  const deviceOptions: DropdownOption[] = detectedDevices.length > 0
    ? detectedDevices.map((d) => ({
        value: d.id,
        label: d.vram_gb != null
          ? `${d.name} (${d.vram_gb.toFixed(1)} GB)`
          : d.name,
      }))
    : [
  ...
  // §11.7 — detected GPU VRAM for OOM check
  const selectedDevice = detectedDevices.find((d) => d.id === device);
  const gpuVramGb = selectedDevice?.vram_gb ?? null;
  // §11.15 [Gap G] — torch.compile is experimental on ROCm; disable the toggle
  const isROCm = selectedDevice?.type === "rocm";
  ...
  // §11.8 — OOM check
  const isOom = gpuVramGb !== null && vramEst > gpuVramGb;
  ...
  const handleLaunch = useCallback(async () => {
    ...
    const payload: TrainingStartMessage = {
      type: "training.start",
      run_id: runId,
      config: {
        architecture,
        upscale_factor: hyperparameters.scale,
        // §11.12 P0-Fix-5: pretrained_path is string | null
        pretrained_path: pretrainedPath ?? null,
      },
      training_config: {
        num_epochs: schedule.totalEpochs,
        batch_size: hyperparameters.batchSize,
        learning_rate: hyperparameters.learningRate,
        scheduler: hyperparameters.lrScheduler,
      ...
      paths: {
        training_data: hrPath,
        // §11.12 P0-Fix-4: null when strategy === "none"
        validation_data: (strategy === "none" ? null : validationPath) as unknown as string,
        checkpoint_dir: checkpointDir,
        log_dir: outputDir,
      },
      // §11.11 — resumeFrom (camelCase) serialized as resume_from (snake_case)
      resume_from: resumeFrom ?? null,
    };
    await sendToSidecar(payload);
  }, [
    architecture, hyperparameters, pretrainedPath, schedule,
    fp16, compile, tensorboard, augmentations, hrPath,
    validationPath, strategy, checkpointDir, outputDir, resumeFrom,
  ]);
        ...
        <Panel title="Run Configuration">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div
              ...
              title={isROCm ? "torch.compile is experimental on AMD ROCm — enabled in payload requires explicit override" : undefined}
            >
              <Toggle on={compile} onChange={() => setCompile(!compile)} disabled={isROCm} />
              <span style={{ fontSize: 11, color: isROCm ? "var(--amber)" : "var(--muted)" }}>
                torch.compile{isROCm ? " (ROCm)" : ""}
              </span>
            </div>
