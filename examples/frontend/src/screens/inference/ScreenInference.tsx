...
import type { InferenceRunMessage } from "../../lib/ipc-types";
...
function buildOutputPath(inputPath: string, outputDir: string, format: string): string {
  const sep = outputDir.includes("\\") ? "\\" : "/";
  const stem = stemOf(basename(inputPath));
  return `${outputDir}${sep}${stem}_sr.${format}`;
}
...
const ARCH_OPTIONS = ["Real-ESRGAN", "SwinIR", "HAT", "EDSR"];
...
function SettingsPanel({ onRun }: { onRun: () => void }) {
  ...
  useEffect(() => {
    ...
    sendToSidecar({
      ...
      run_id: run.run_id,
      checkpoint_dir: run.paths.checkpoint_dir,
    }).catch(() => { /* best-effort; sidecar may not be running */ });
  ...
  // §20.11 — Pre-flight output directory validation (inline, not a toast)
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    invoke<boolean>("validate_dir_writable", { path: dir })
      .then((ok) => {
        if (!cancelled) {
          setOutputDirError(ok ? null : "Directory does not exist or is not writable");
        }
      })
      .catch(() => {
        if (!cancelled) setOutputDirError("Could not validate directory");
      });
...
export function ScreenInference() {
  ...
  const handleRun = useCallback(async () => {
    ...
    const outputPath =
      s.outputDir
        ? buildOutputPath(s.inputPath, s.outputDir, s.outputFormat)
        : buildOutputPath(
            s.inputPath,
            s.inputPath.substring(
              0,
              Math.max(s.inputPath.lastIndexOf("/"), s.inputPath.lastIndexOf("\\"))
            ),
            s.outputFormat
          );
    ...
    const msg: InferenceRunMessage = {
      ...
      checkpoint_path: s.checkpointPath,
