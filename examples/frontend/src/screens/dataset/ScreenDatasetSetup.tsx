...
interface VideoExtractionModeProps {
  ...
  extractionProgress: DatasetProgressMessage | null;
  lrScalingProgress: { done: number; total: number } | null;
}

function VideoExtractionMode({ videoFiles, onAddFiles, extractionProgress, lrScalingProgress }: VideoExtractionModeProps) {
...
interface OnTheFlyModeProps {
  ...
  onHrPath: (p: string) => void;
  onLrPath: (p: string) => void;
  encodingProgress: DatasetProgressMessage | null;
  sync: "frame" | "time";
...
export function ScreenDatasetSetup() {
  ...
  const [splitResult, setSplitResult] = useState<{ trainingCount: number; validationCount: number } | null>(null);
  const [videoFiles, setVideoFiles] = useState<VideoFile[]>([]);
  const [lrGenerationProgress, setLrGenerationProgress] = useState<LrGenerationProgress | null>(null);
  const [encodingProgress, setEncodingProgress] = useState<DatasetProgressMessage | null>(null);
  const [lrScalingProgress, setLrScalingProgress] = useState<{ done: number; total: number } | null>(null);
  const [onTheFlySync, setOnTheFlySync] = useState<"frame" | "time">("frame");

  const mode = type as DatasetMode;

  // Wire IPC events for dataset progress and split result
  useEffect(() => {
    let unlisten: (() => void) | null = null;

    listen<IPCMessage>("sidecar-message", ({ payload }) => {
      if (payload.type === "dataset.progress") {
        const msg = payload as DatasetProgressMessage;
        setExtractionProgress({
          framesDone: msg.frames_done,
          framesTotal: msg.frames_total,
          fps: msg.fps,
          etaSec: msg.eta_sec,
        });
        if (mode === "on_the_fly") {
          setEncodingProgress(msg);
        } else if (mode === "video_extract") {
          setLrScalingProgress({ done: msg.frames_done, total: msg.frames_total });
        } else {
          setLrGenerationProgress({
            framesDone: msg.frames_done,
            framesTotal: msg.frames_total,
            done: msg.frames_done >= msg.frames_total,
          });
        }
      }
      if (payload.type === "dataset.split.result") {
        const msg = payload as DatasetSplitResultMessage;
        setValidationPath(msg.validation_path);
        setSplitResult({ trainingCount: msg.training_count, validationCount: msg.validation_count });
      }
    }).then((fn) => { unlisten = fn; });

    return () => { unlisten?.(); };
  }, [mode, setExtractionProgress, setValidationPath]);
  ...
  const handleStartExtraction = async () => {
    ...
    await sendToSidecar({
      type: "dataset.create",
      mode: "video_extract",
      source_paths: videoFiles.map((f) => f.path),
      output_hr_path: hrOut,
      output_lr_path: lrOut,
      scale_factor: scale,
      kernel,
      naming_pattern: namingPattern || "%06d",
      ffmpeg_fps: null,
      sync_mode: null,
    });
  };
  ...
  const handlePreprocess = async () => {
    ...
    await sendToSidecar({
      ...
      source_paths: [hrPath],
      output_hr_path: hrOut,
      output_lr_path: lrPath,
      scale_factor: scale,
      kernel,
      naming_pattern: namingPattern || "%06d",
      ffmpeg_fps: null,
      sync_mode: onTheFlySync,
    });
  };
          ...
          {mode === "video_extract" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <VideoExtractionMode
                ...
                extractionProgress={
                  extractionProgress
                    ? { type: "dataset.progress", frames_done: extractionProgress.framesDone, frames_total: extractionProgress.framesTotal, fps: extractionProgress.fps, eta_sec: extractionProgress.etaSec }
                    : null
                }
